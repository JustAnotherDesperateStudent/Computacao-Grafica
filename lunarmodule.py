...
offset = 0
print("Pressione as setas para esquerda e direita para rotação.\nSegure seta para cima para user a propulsão")
...
while(running):
  ...
  if (capsule.fuel < 20):
    ...
    announce.text = "Combustivel baixo!"
  ...
  for i in range(len(pad)):
    ...
    horsep = abs(capsule.pos.x - pad[i].pos.x)  # Distancia horizontal entre o módulo e a zona
    versep = capsule.pos.y - 1 - (pad[i].pos.y + pad[i].axis.y)   # Distancia vertical entre o módulo e a zona
    ...
    if ((horsep < 0.85 * pad[i].radius) and (abs(versep) < 0.1 * capsule.axis.mag)): 
      ...
      if (abs(capsule.vel.x) < 1.0 and abs(capsule.vel.y) < 1.5 and capsule.angle < capsule.rotangle):
        announce.text = "Pouso bem sucedido!"
        ...
        if (capsule.angle > capsule.rotangle):
          announce.text += "\nAngulo de rotacao muito alto!"
        if (abs(capsule.vel.x) >= 1.0): 
          announce.text += "\nVelocidade horizontal muito alta!"
        if (abs(capsule.vel.y) >= 1.5): 
          announce.text += "\nVelocidade vertical muito alta!"
  ...
  data.text += '\nVelocidade Horizontal: ' + str(round(10*capsule.vel.x)/10) + " m/s"
  data.text += '\nVelocidade Vertical: ' + str(round(10*capsule.vel.y)/10) + " m/s"
  data.text += '\nOrientacao: ' + str(round(-10*capsule.angle * 5.625 % 360)) + " deg"
  data.text += "\nCombustivel: " + str(round(10*capsule.fuel)/10) + " units"
